...
const IncomingRequestManager = () => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/swaps/my-swaps', {
  ...
  const handleAccept = async (swapId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/swaps/${swapId}/accept`, {
        method: 'PUT',
        headers: { 'x-auth-token': localStorage.getItem('token') || '' },
      });
        ...
        requests.map((request) => (
          <div key={request._id} className="card mb-3">
            <div className="card-body">
              <p className="mb-2">
                <strong>{request.requester?.username || 'A collector'}</strong> wants to swap:
              </p>
